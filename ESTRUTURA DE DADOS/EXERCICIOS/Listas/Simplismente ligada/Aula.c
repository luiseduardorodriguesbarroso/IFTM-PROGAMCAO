    ...
    int removerFimLSLI(ListaSLI *pontlista){
        ...
        while(aux->proximo != NULL){
            anterio = aux; //Anterior = anterio do aux
            aux = aux->proximo;
        } 
        int valor = aux->valor; //Vai acessar o ultimo valor da lista

        anterio->proximo = NULL; // Anterior = aux
        free(aux);

        pontlista->tamanho--; //Reduzo a minha lista 
